...
def submit_task(task_id: int, hg_repo_id: str, base_model: str, gpu_type: str, revision: str):
    payload = json.dumps(
        {
            "task_id": task_id,
            "data": {
...
def submit_results_with_metrics(
    task_id: int, hg_repo_id: str, base_model: str, gpu_type: str, revision: str, metrics: dict
):
    payload = json.dumps(
        {
            "task_id": task_id,
    ...
    if response.status_code != 200:
        raise Exception(f"Failed to submit task with metrics: {response.text}")
    return response.json()
